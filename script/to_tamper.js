...
function generate() {
  ...
  result +=
    "window.popuphtml=String.raw`<div id='popup' style='padding:10px;display: inline-block;position: absolute;z-index: 100;background: white;transform: translate(0, 100%);border-style: double;bottom: 0;left: 0;'>" +
    fs.readFileSync(popup_uri, "utf8") +
    "</div>`\n";
...
function inject(src) {
  ...
  return (
    "$(document.body).append(String.raw`" +
    $("<script>").text(data).get(0).outerHTML +
    "`)\n"
  );
}
